...
{
  float value = (float)val.bytes[0] + ((unsigned char)val.bytes[1])/256.0;
  printf("%4.1f ", value);
}
...
int smc_main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "qfhk:lrw:v")) != -1)
    {
        switch(c)
        {
	case 'q':
        ...
        case 'w':
            op = OP_WRITE;
	    longvalue = strtol(optarg, NULL, 0);
            {
                int i;
                char c[3];
    ...
    case OP_WRITE:
        ...
        {
	  SMCVal_t tmp_val;
            result = SMCReadKey(key, &tmp_val);
            if (result == kIOReturnSuccess) {
	      int i;
	      if (strcmp(tmp_val.dataType, DATATYPE_FPE2) == 0) {
		longvalue *= 4;
		for (i = 0; i < tmp_val.dataSize; i++) {
		  val.bytes[i] = (longvalue>>((tmp_val.dataSize-i-1)*8)) & 0xff;
		  //printf ("%ld, %d %d\n", longvalue, i , val.bytes[i]);
		}
		val.dataSize = tmp_val.dataSize;
	      }
	    }
            sprintf(val.key, key);
